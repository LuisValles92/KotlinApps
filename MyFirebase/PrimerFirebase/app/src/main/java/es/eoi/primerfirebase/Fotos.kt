...
class Fotos(private var activity: Activity, private var imageView: ImageView) {
    ...
    private fun pedirPermisos() {
        val deboProveerContexto =
            ActivityCompat.shouldShowRequestPermissionRationale(activity, permisoCamera)
    ...
    private fun solicitudPermisos() {
        activity.requestPermissions(
            arrayOf(permisoCamera, permisoWriteStorage, permisoReadStorage),
        ...
        when (requestCode) {
            ...
            SOLICITUD_SELECCIONAR_FOTO -> {
                if (grantResults.size > 0 &&
                    ...
                    Toast.makeText(
                        activity.applicationContext,
                        "No diste permiso para acceder a tus fotos",
                        Toast.LENGTH_SHORT
                    ).show()
                }
            }
    ...
    private fun dispararIntentSeleccionarFoto() {
        ...
        activity.startActivityForResult(
            Intent.createChooser(intent, "Seleccionar una foto"),
            SOLICITUD_SELECCIONAR_FOTO
        )
    }

    fun activityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        //Log.d("ACTIVITY_RESULT", resultCode.toString()) //resultCode -1 V 0 X
    ...
    private fun crearArchivoImagen(): File {
        ...
        var directorioPictures = File(directorio.absolutePath + "/Pictures")
